...
----
''''
Now, if there's a worker installing, we should listen to the state change by creating a new method called *trackInstalling*. +
----
if (reg.installing) {
...
''''
If there is an update, we will use trackInstalling again.
----
reg.addEventListener('updatefound', function() {
    indexController._trackInstalling(reg.installing);
    return;
